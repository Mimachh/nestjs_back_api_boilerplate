...
export class AuthService {
  ...
  async register({ registerBody }: { registerBody: CreateUserDto }) {
    const { email, firstName, password, role, address } = registerBody;
    ...
    if (role.includes('vendor') || role.includes('pro')) {
      const { street, city, zip, country } = address[0];
    ...
    const newUser = await this.prisma.user.create({
      data: {
        ...
        // relation to user->profile->address
        profile:
          role.includes('vendor') || role.includes('pro')
            ? {
                create: {
                  address: {
                    create: {
